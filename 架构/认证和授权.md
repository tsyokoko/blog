...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/20210604160908352.png)

授权：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/20210604161032412.png)
...
![RBAC](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/RBAC.png)

**在 RBAC 中，权限与角色相关联，用户通过成为适当角色的成员而得到这些角色的权限。这就极大地简化了权限的管理。**

本系统的权限设计相关的表如下（一共 5 张表，2 张用户建立表之间的联系）：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/%E6%95%B0%E6%8D%AE%E5%BA%93%E8%AE%BE%E8%AE%A1-%E6%9D%83%E9%99%90.png)

通过这个权限模型，我们可以创建不同的角色并为不同的角色分配不同的权限范围（菜单）。

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/%E6%9D%83%E9%99%90%E7%AE%A1%E7%90%86%E6%A8%A1%E5%9D%97.png)

通常来说，如果系统对于权限控制要求比较严格的话，一般都会选择使用 RBAC 模型来做权限控制。

## 什么是 Cookie ? Cookie 的作用是什么?

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/20210615162505880.png)
...
1. 使用 `Cookie` 保存 `Session` 或者 `Token` ，向后端发送请求的时候带上 `Cookie`，这样后端就能取到 `Session` 或者 `Token` 了。这样就能记录用户当前的状态了，因为 HTTP 协议是无状态的。
1. `Cookie` 还可以用来记录和分析用户行为。举个简单的例子你在网上购物的时候，因为 HTTP 协议是没有状态的，如果服务器想要获取你在某个页面的停留状态或者看了哪些商品，一种常用的实现方式就是将这些信息存放在 `Cookie`
1. ......
...
1. 当用户向后端发起请求的时候会把 `SessionID` 带上，这样后端就知道你的身份状态了。

关于这种认证方式更详细的过程如下：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/session-cookie.png)
...
1. 服务器可以将存储在 `Cookie` 上的 `SessionID` 与存储在内存中或者数据库中的 `Session` 信息进行比较，以验证用户的身份，返回给用户客户端响应信息的时候会附带用户当前的状态。
...
1. 单独使用一个所有服务器都能访问到的数据节点（比如缓存）来存放 Session 信息。为了保证高可用，数据节点尽量要避免是单点。
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/20210615161108272.png)
...
1. **Payload** : 用来存放实际需要传递的数据
1. **Signature（签名）** ：服务器通过`Payload`、`Header`和一个密钥(`secret`)使用 `Header` 里面指定的签名算法（默认是 HMAC SHA256）生成。
...
![jwt](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/jwt.png)
...
![sso](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/sso.png)
...
![](/Users/mbpzy/Downloads/JavaGuide/docs/system-design/authority-certification/images/basis-of-authority-certification/%25E5%25BE%25AE%25E4%25BF%25A1%25E6%2594%25AF%25E4%25BB%2598-fnglfdlgdfj.png)

下图是 [Slack OAuth 2.0 第三方登录](https://api.slack.com/legacy/oauth)的示意图：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/20210615151716340.png)
