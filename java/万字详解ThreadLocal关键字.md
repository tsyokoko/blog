...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/1-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/2-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/3-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/4-1585712.png)
...
![image.png](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/5-1585712.png)

如果我们的**强引用**不存在的话，那么 `key` 就会被回收，也就是会出现我们 `value` 没被回收，`key` 被回收，导致 `value` 永远存在，出现内存泄漏。

### `ThreadLocal.set()`方法源码详解

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/6-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/8-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/7-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/9-1585712.png)

这里直接将数据放到该槽位即可。

**第二种情况：** 槽位数据不为空，`key`值与当前`ThreadLocal`通过`hash`计算获取的`key`值一致：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/10-1585712.png)

这里直接更新该槽位的数据。

**第三种情况：** 槽位数据不为空，往后遍历过程中，在找到`Entry`为`null`的槽位之前，没有遇到`key`过期的`Entry`：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/11-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/12-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/13-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/14-1585712.png)
...
![preview](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/view.png)


**向后遍历过程中，如果没有找到相同key值的Entry数据：**

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/15-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/16-1585712.png)
...
1. 碰到一个过期的桶，执行替换逻辑，占用过期桶
1. 查找过程中，碰到桶中`Entry=null`的情况，直接使用

接着就是执行`for`循环遍历，向后查找，我们先看下`nextIndex()`、`prevIndex()`方法实现：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/17-1585712.png)
...
1. `for`循环执行完毕，继续往下执行说明向后迭代的过程中遇到了`entry`为`null`的情况
   3.1 在`Entry`为`null`的桶中创建一个新的`Entry`对象
   3.2 执行`++size`操作
1. 调用`cleanSomeSlots()`做一次启发式清理工作，清理散列数组中`Entry`的`key`过期的数据
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/18-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/19-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/20-1585712.png)

我们假设`expungeStaleEntry(3)` 来调用此方法，如上图所示，我们可以看到`ThreadLocalMap`中`table`的数据情况，接着执行清理操作：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/21-1585712.png)

第一步是清空当前`staleSlot`位置的数据，`index=3`位置的`Entry`变成了`null`。然后接着往后探测：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/22-1585712.png)

执行完第二步后，index=4的元素挪到index=3的槽位中。

继续往后迭代检查，碰到正常数据，计算该数据位置是否偏移，如果被偏移，则重新计算`slot`位置，目的是让正常数据尽可能存放在正确位置或离正确位置更近的位置

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/23-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/24-1585712.png)

接着看看具体的`resize()`方法，为了方便演示，我们以`oldTab.len=8`来举例：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/25-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/26-1585712.png)

**第二种情况：** `slot`位置中的`Entry.key`和要查找的`key`不一致：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/27-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/28-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/29-1585712.png)
...
![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/30-1585712.png)

图中的`requestId`即为我们各个系统链路关联的`traceId`，系统间互相调用，通过这个`requestId`即可找到对应链路，这里还有会有一些其他场景：

![](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/31-1585712.png)
