...
原来是 String 重写了 Object 的 equals 方法，把引用比较改成了值比较。同样的，当我们进入 String 的 equals 方法，找到了答案，代码如下：
...
String 不属于基础类型，基础类型有 8 种：byte(1个字节)、boolean(1个字节)、char(1个字节)、short(2个字节)、int(2个字节)、float(4个字节)、long(8个字节)、double(8个字节)，而 String 属于对象。

Java中的字符串String最大长度，编译期如果是javac编译就是65534。如果绕过javac编译的限制`((String)constValue).length() < Pool.MAX_STRING_LENGTH)` ，其最大长度可以达到u2类型变达的最大值65535，Java中的字符串String最大长度运行期大约4G。
...
线程的状态：NEW 尚未启动、RUNNABLE 正在执行中、BLOCKED 阻塞的（被同步锁或者IO锁阻塞）、WAITING 永久等待状态、TIMED_WAITING 等待指定的时间重新被唤醒的状态、TERMINATED 执行完成
...
单线程线程池、固定缓存线程池、固定工作线程数线程池、可调度单线程线程池、可调度线程池、工作窃取线程池、原始线程池。
...
空指针异常、数组下标越界异常、数据类型转换异常、文件/类/方法不存在异常、IO异常
...
plugin 方法是插件用于封装目标对象的，通过该方法我们可以返回目标对象本身，也可以返回一个它的代理，可以决定是否要进行拦截进而决定要返回一个什么样的目标对象，官方提供了示例：
...
在查询性能上，使用 explain 查看 SQL 是如何执行查询语句的，从而分析你的索引是否满足需求。

```sql
explain select * from table where type=1
```

主要看查询类型`type`，观察是否使用到索引，常见的类型有ALL、index、range、 ref、eq_ref、const、system、NULL（从左到右，性能从差到好）

其次是使用到的主键`Key`与`Extra`列，后者包括`Using where`、`Using temporary`以及`Using filesort`
...
EPEATABLE-READ：可重复读，默认级别，保证多次读取同一个数据时，其值都和事务开始时候的内容是一致，禁止读取到别的事务未提交的数据（会造成幻读）。
...
InnoDB 引擎：InnoDB 引擎提供了对数据库 acid 事务的支持，并且还提供了行级锁和外键的约束，它的设计的目标就是处理大数据容量的数据库系统。MySQL8之前，运行的时候，InnoDB 会在内存中建立缓冲池，用于缓冲数据和索引。但是该引擎是不支持全文搜索，同时启动也比较的慢，它是不会保存表的行数的，所以当进行 select count(\*) from table 指令的时候，需要进行扫描全表。由于锁的粒度小，写操作是不会锁定全表的,所以在并发度较高的场景下使用会提升效率的。
...
3.根据返回修改记录条数来判断当前更新是否生效，如果成功改动0条数据，说明version发生了变更，这时候可以根据自己业务逻辑来决定是否需要回滚事务
```
...
log-slow-queries=/var/lib/mysql/slowquery.log #指定日志文件存放位置，可为空设置为默认
...
Redis 使用场景：记录帖子点赞数、点击数、评论数、缓存近期热帖、缓存文章详情信息、记录用户会话信息。
...
根据从全数据集（server. db[i]. dict）与已设置过期时间的数据集（server. db[i]. expires）不同，有`volatile`和`allkeys`；根据具体策略不同，有近期最少使用`lru`、随机`random`；除此之外`volatile`还有`ttl`策略，从已设置过期时间的数据集中选择挑选将要过期的数据淘汰；以及永不驱逐策略。
...
在介绍双亲委派模型之前先说下类加载器。对于任意一个类，都需要**由加载它的类加载器和这个类本身一同确立在 JVM 中的唯一性**，每一个类加载器，都有一个独立的类名称空间。**类加载器就是根据指定全限定名称将 class 文件加载到 JVM 内存，然后再转化为 class 对象**。
...
可达性分析：从 GC Roots 开始向下搜索，搜索所走过的路径称为引用链。当一个对象到GC Roots 没有任何引用链相连时，则证明此对象是可以被回收的。
