...
![jdk1.8之前的内部结构](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/jdk1.8%E4%B9%8B%E5%89%8D%E7%9A%84%E5%86%85%E9%83%A8%E7%BB%93%E6%9E%84.png)
...
1. 如果定位到的数组位置有元素就和要插入的 key 比较，如果 key 相同就直接覆盖，如果 key 不相同，就判断 p 是否是一个树节点，如果是就调用`e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value)`将元素添加进入。如果不是就遍历链表插入(插入的是链表尾部)。

![ ](https://tsyokoko-typora-images.oss-cn-shanghai.aliyuncs.com/img/put%E6%96%B9%E6%B3%95.png)
